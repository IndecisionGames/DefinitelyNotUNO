...
func update_hand(name, card_count, is_current_player, is_uno):
	...
	if is_current_player:
		name_label.set("theme_override_colors/font_color", Values.current_player)
		name_label.set_scale(Vector2(1.5, 1.5))
	else:
		name_label.set("theme_override_colors/font_color", Values.white)
		name_label.set_scale(Vector2(1, 1))

	if is_uno:
		card_count_label.set("theme_override_colors/font_color", Values.red)
		count_text = "UNO"
	else:
		card_count_label.set("theme_override_colors/font_color", Values.white)
...
func play_draw_animation(n):
	...
	var initial_rotation = get_parent().rotation * -1
	for _i in range(n):
